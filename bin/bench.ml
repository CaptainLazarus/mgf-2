...
let all_head_grammars (g : Types.grammar) : Types.grammar list =
  ...
  let rec cart = function
    | [] -> [ [] ]
    | ps :: rest ->
        List.concat_map (fun p -> List.map (fun t -> p :: t) (cart rest)) ps
  in
...
let head_label (g : Types.grammar) =
  let ps =
    List.map
      (fun (p : Types.production) -> string_of_int p.head_pos)
      g.productions
  in
  "(" ^ String.concat "," ps ^ ")"
...
let run_experiment name (orig : Types.grammar) (input : string list) =
  ...
  let orig_rules, orig_tbl, _ =
    List.find (fun (_, _, l) -> l = orig_label) results
  in
  ...
  Printf.printf "\n=== %s | input: [%s] (n=%d) | %d configs ===\n" name
    (String.concat " " input) (List.length input) (List.length all);
  List.iter
    (fun (p : Types.production) ->
      Printf.printf "  prod %d: %s -> %-20s (len=%d)\n" p.index p.lhs
        (String.concat " "
           (List.map
              (function Types.Terminal t -> t | Types.Nonterminal n -> n)
              p.rhs))
        (List.length p.rhs))
    orig.productions;
  Printf.printf "\n";
  ...
  Printf.printf "  cover rules : min=%-3d  max=%-3d  original=%-3d (%s)\n"
    ...
    (if orig_rules = min_rules then "minimum"
     else if orig_rules = max_rules then "MAXIMUM"
     else "middle");
  Printf.printf "  table items : min=%-3d  max=%-3d  original=%-3d (%s)\n"
    min_tbl max_tbl orig_tbl
    (if orig_tbl = min_tbl then "minimum"
     else if orig_tbl = max_tbl then "MAXIMUM"
     else "middle")

let () =
  run_experiment "GCL" Grammars.grammar_gcl
    [ "det"; "n"; "cl"; "v"; "det"; "n" ];
  run_experiment "Arith" Grammars.grammar_arith [ "n"; "+"; "n"; "+"; "n" ];
  run_experiment "Astar" Grammars.grammar_astar [ "a"; "a"; "a" ]
