...
let () =
  ...
  let inputs =
    ...
    ; ["LPAREN"; "ATOM"; "RPAREN"]
    ; ["LPAREN"; "RPAREN"]
    ; ["LPAREN"; "ATOM"; "DOT"]   (* invalid *)
    ]
  in

  (* 3. Recognise each input and collect all parse trees — list of lists *)
  let results : (string list * Htable.tree list) list =
    List.map (fun input ->
      let tbl = Htable.recognize_with pg input in
      let trees = Htable.reconstruct_trees_omit tbl grammar.start in
      (input, trees))
      inputs
  in
