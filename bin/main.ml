...
type grammar_choice = Lisp of string list | C [@@warning "-37"]
...
let () =
  let grammar =
    grammar_file active_grammar
    |> Grammar_reader.extract_grammar |> Grammar_converter.convert_grammar
  in
  ...
  let tbl =
    Htable.show ~roots:true ~grammar:false ~cover:true ~table:true ~cells:false
      ~result:false tbl
  in
